...
class ConfigurationProvider(Protocol):
    """Protocol for configuration access supporting test mocking."""
    
    def get_ignore_patterns(
        self, config: Dict[str, Any], experiment: Optional[str] = None
    ) -> List[str]:
...
class DefaultConfigurationProvider:
    """Default implementation of ConfigurationProvider using standard configuration functions."""
    
    def get_ignore_patterns(
        self, config: Dict[str, Any], experiment: Optional[str] = None
    ) -> List[str]:
        """Get ignore patterns from configuration."""
        return get_ignore_patterns(config, experiment)
    
    def get_mandatory_substrings(
        self, config: Dict[str, Any], experiment: Optional[str] = None
    ) -> List[str]:
        """Get mandatory substrings from configuration."""
        return get_mandatory_substrings(config, experiment)
    
    def get_dataset_info(self, config: Dict[str, Any], dataset_name: str) -> Dict[str, Any]:
        """Get dataset information from configuration."""
        return get_dataset_info(config, dataset_name)
    
    def get_experiment_info(self, config: Dict[str, Any], experiment_name: str) -> Dict[str, Any]:
        """Get experiment information from configuration."""
        return get_experiment_info(config, experiment_name)
    ...
    def get_extraction_patterns(
        ...
        """Get extraction patterns from configuration."""
        return get_extraction_patterns(config, experiment, dataset_name)
...
class ConfigDiscoveryEngine:
    ...
    def discover_files_with_config(
        ...
        """
        Discover files using configuration-aware filtering with enhanced error handling.
        ...
        """
        # Enhanced parameter validation for improved test coverage
        ...
        # Validate experiment exists if specified
        if experiment and experiment not in config.get("experiments", {}):
            logger.warning(f"Experiment '{experiment}' not found in configuration, using project-level settings only")
        
        logger.debug(
            f"Discovering files with config for experiment={experiment}, "
    ...
    def discover_experiment_files(
        ...
        """
        Discover files related to a specific experiment with enhanced validation and logging.
        
        This uses the experiment's dataset definitions and filter settings
        to find relevant files with improved error handling and test observability.
        ...
        logger.info(f"Starting experiment file discovery for experiment '{experiment_name}'")
        
        try:
            # Get experiment information with enhanced error handling
    ...
    def discover_dataset_files(
        ...
        """
        Discover files related to a specific dataset with enhanced validation and logging.
        
        This uses the dataset's date-vial definitions to find relevant files
        with improved error handling and test observability.
        ...
        logger.info(f"Starting dataset file discovery for dataset '{dataset_name}'")
        
        try:
            # Get dataset information with enhanced error handling
            dataset_info = self.config_provider.get_dataset_info(config, dataset_name)
...
# Backward-compatible module-level functions with enhanced implementation
def discover_files_with_config(
    config: Dict[str, Any],
    directory: Union[str, List[str]],
    pattern: str,
